...
def evaluate_test(monkey, worry_level)
  worry_level = (do_operation(monkey['operation'], worry_level) / 3).floor
...
20.times do
  monkeys.each do |monkey_number, monkey|
    monkey['items'].each do |worry_level|
      target_monkey, new_worry_level = evaluate_test(monkey, worry_level)
